...
EOT;
    ...
    public function generateListicle(
        ...
        $kwRule = ($keywords !== null && trim($keywords) !== '')
            ? "\n- SEO: органично вплети 2–4 из этих поисковых запросов (без переспама, не списком): {$keywords}"
            : '';
...
Требования:
- Вступление: 1–2 абзаца — зачем рейтинг и как выбирали, без воды.
- Для каждого бренда секция: заголовок «## {место}. {название}», затем 2–4 абзаца СТРОГО по его фактам (ассортимент, особенности, для кого).
- Не выдумывай цены, даты, цифры и сравнения, которых нет в блоке бренда.
- Заключение: короткий абзац-итог.
- Живой человеческий язык, без штампов и слов «уникальный», «инновационный», «лидирующий», «передовой».
- НЕ добавляй H1 (# …) — начни сразу со вступления.{$kwRule}
