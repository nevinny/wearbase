...
class LlmService
{
    ...
    /**
     * Разговорные прилагательные городов для generateCityHub(): 45% гео-спроса СПб
     * идёт на «питерск-» (docs/geo_city_demand_2026_09.md §4), а формальное «Санкт-
     * Петербург» на странице встречается один раз — то же расхождение ожидаемо у
     * любого города. Прилагательное не выводится алгоритмически (мск→«мские» и
     * подобное — бессмыслица), только ручной список. Живёт здесь (не в команде рядом
     * с CITY_ALIASES), потому что $city — единственный параметр generateCityHub,
     * из которого его можно взять, а сигнатуру метода трогать нельзя.
     */
    private const CITY_ADJECTIVES = [
        'Санкт-Петербург'  => 'питерские',
        'Москва'           => 'московские',
        'Екатеринбург'     => 'екатеринбургские',
        'Нижний Новгород'  => 'нижегородские',
        'Новосибирск'      => 'новосибирские',
        'Пермь'            => 'пермские',
        'Челябинск'        => 'челябинские',
        'Краснодар'        => 'краснодарские',
        'Казань'           => 'казанские',
        'Красноярск'       => 'красноярские',
        'Ростов-на-Дону'   => 'ростовские',
    ];
...
- Первое предложение — прямое определение бренда без вступлений: «{$brandName} — [ниша/категория] бренд из [город]» (сразу факт, не эпитет и не история)
- Тон: информативный, без восторженных фраз
- Структура: 3–4 абзаца, разделённых пустой строкой, каждый — одна тема
- Первое предложение КАЖДОГО абзаца — самостоятельный ответ (40–60 слов), понятный без остального текста, на конкретный вопрос покупателя (что производит бренд / из чего сделано / для кого / где искать) — без заголовков, просто первая фраза абзаца
- Привязывай характеристики к применению, а не перечисляй списком (не «хлопок, лён, шерсть», а как это влияет на носку/уход/сезон)
- Не трать слова на общеизвестное об индустрии в целом — весь объём на конкретные и редкие факты именно об этом бренде
...
ЖЁСТКИЕ ПРАВИЛА:
- Только ФАКТЫ выше. Не выдумывай историю города, улицы, шоурумы, цены, даты.
- Число брендов каталога («более N марок») не упоминай: его выводит сама страница.
  Числа из ФАКТОВ (год основания, размерный ряд) — можно.
- Назови 3–6 брендов из ФАКТОВ, у каждого один проверяемый факт из его описания.
  Названия переписывай символ в символ: не сокращай, не дописывай город, не выдумывай.
- Первое предложение intro — ответ на «какие бренды одежды есть в {$city}»: преобладающие
  стили, без числа брендов.
- {$adjectiveRule}
- intro: 3 абзаца, 900–1500 символов, HTML — только теги <p>.
- Средняя длина предложения до 15 слов, ни одного длиннее 25. Тире «—» максимум одно
  на абзац. В каждом абзаце — минимум один факт из ФАКТОВ.
- Только русский язык; латиница — лишь в названиях брендов.
- Запрещено: рекламные штампы («широкий ассортимент», «мир моды», «на любой вкус»,
  «уникальный стиль», «своя специфика»); абзацы-заполнители без факта; текст про
  покупку напрямую, сайты и соцсети (это уже есть в шаблоне страницы).
- Про поисковые запросы и пользователей сервиса не пиши вообще: фразы даны для
  подбора лексики, а не как тема. Ни одной фразы дословно — перестраивай порядок слов.
- FAQ: три пары. Не больше одного вопроса общего вида про город — остальные называют
  конкретный бренд и его деталь. Ответ 2–4 предложения, первое — самодостаточное.
  Отвечай только по ФАКТАМ; нет ответа в фактах — меняй вопрос.
- meta_description называет хотя бы один бренд из ФАКТОВ.
- h1 — «Бренды одежды <город в родительном падеже>», без «в городе», ≤ 70 символов.
  meta_title ≤ 70, meta_description ≤ 160.
...
EOT;
    ...
    /**
     * Контент гео-хаба /{_locale}/cities/{slug}: h1, мета, intro и FAQ — строго из
     * фактов каталога ($facts) и реальных поисковых фраз по городу ($phrases).
     *
     * Зачем grounded: единственные факты о городе, которыми мы располагаем, — состав
     * каталога (бренды, стили, число). Всё остальное («город моды», «в СССР шили») —
     * вода, за которую страницу и держат на позиции 8 вместо топ-3.
     *
     * @param string[] $phrases поисковые фразы по городу, по убыванию спроса
     * @return array{h1: ?string, meta_title: ?string, meta_description: ?string, intro: ?string, faq: array<int, array{question: string, answer: string}>}
     */
    public function generateCityHub(string $city, string $facts, array $phrases): array
    {
        // Города следующего эшелона (Новосибирск, Пермь…) собирают 4-44 показа —
        // фраз почти нет, и с пустым/жидким списком модель добивает объём шаблоном
        // вместо фактов (docs/geo_city_demand_2026_09.md §7). Меньше 3 фраз — не
        // подаём их вовсе, а не только на действительно пустой список.
        $phrasesList = count($phrases) < 3
            ? '(фраз мало или нет — НЕ используй их и не подгоняй текст под них; опирайся только на состав каталога из ФАКТОВ)'
            : implode("\n", array_map(static fn(string $p) => "- {$p}", $phrases));

        $adjective = self::CITY_ADJECTIVES[$city] ?? null;
        $adjectiveRule = $adjective !== null
            ? "В intro и FAQ хотя бы раз используй разговорное прилагательное «{$adjective}» — так пишут в поиске; в h1 и meta_title — только формальное «{$city}»."
            : "В h1 и meta_title используй формальное «{$city}».";

        $prompt = <<<EOT
Город: {$city}

ФАКТЫ из каталога WEARBASE (единственный источник истины):
{$facts}

Поисковые фразы реальных пользователей по этому городу (по убыванию спроса):
{$phrasesList}

Составь контент посадочной страницы «бренды одежды {$city}» в каталоге локальных
российских брендов WEARBASE.

ЖЁСТКИЕ ПРАВИЛА:
- Опирайся ТОЛЬКО на ФАКТЫ выше. Не выдумывай историю города, «модные традиции»,
  улицы, шоурумы, цены, даты и число жителей.
- Число брендов города (сколько их в каталоге, «более N марок») не упоминай нигде в
  тексте и мете — его отдельно выводит страница из реальных данных каталога, и текстовое
  число тут же разойдётся с ним при следующем изменении каталога. Прочие числа, взятые
  из ФАКТОВ (год основания, «более 15 лет на рынке», размерный диапазон), использовать
  можно — они не разъезжаются.
- Назови минимум 3 и максимум 6 конкретных брендов из ФАКТОВ; у каждого — один
  проверяемый факт (стиль или деталь из его anons). Это единственное, что реально
  различает города, — общие фразы про моду не различают.
- Первое предложение intro — самодостаточный ответ на «какие бренды одежды есть в
  городе {$city}»: назови преобладающие стили (без числа брендов). Понятно без заголовка.
- {$adjectiveRule}
- intro: 3–4 абзаца, 1200–1700 символов, HTML только теги <p>. Без списков и заголовков.
  Если фактов мало и после трёх абзацев текста меньше нормы — раздели на 4 абзаца по
  300–400 символов, но НЕ подгоняй абзацы под фиксированные темы (город/бренды/стили/
  итог) — жёсткая структура сама по себе делает тексты городов однотипными.
- Средняя длина предложения — до 15 слов, чередуй короткие и длинные; ни одного
  предложения длиннее 25 слов (длинные монотонные фразы читаются как ИИ-текст).
- Длинное тире «—» — не больше одного на абзац (частый маркер ИИ-письма, за него
  снижают оценку читаемости).
- В каждом абзаце — минимум один проверяемый факт из ФАКТОВ (конкретный стиль с
  количеством, название бренда, деталь из anons), не только общие слова.
- Никаких рекламных штампов («широкий ассортимент», «мир моды», «идеальный выбор»,
  «на любой вкус», «свой уникальный стиль», «своя специфика»).
- Не пиши про покупку напрямую без наценки маркетплейсов, про сайты и соцсети
  брендов — этот блок уже есть в шаблоне страницы, повторение его текстом делает
  города неотличимыми друг от друга.
- НИКОГДА не пиши о самих поисковых запросах и о пользователях сервиса. Фразы даны,
  чтобы подобрать лексику текста, а не как тема для рассказа. Формулировки вида
  «пользователи часто ищут…», «популярный запрос касается…», «многие ищут через наш
  сервис», «поисковые запросы касаются…» запрещены полностью: читатель пришёл за
  брендами города, а не за отчётом по семантике.
- Названия брендов переписывай из ФАКТОВ символ в символ. Не сокращай, не дописывай
  город к названию и не выдумывай вариантов: «NIKA-WEAR» нельзя превращать в
  «NIKA-WEroll», а «Molotov» в отдельный бренд «Molotov Perm».
- Пиши только по-русски (латиница допустима лишь в названиях брендов). Ни одного
  символа других алфавитов.
- Поисковые фразы вплетай естественно и НЕ дословно: перестраивай порядок слов под
  живую речь. Склейка вида «молотов одежда пермь» или «Molotov Perm» прямо в тексте —
  это переспам, так нельзя; пиши «пермская марка Molotov». Одну фразу — максимум один
  раз на весь текст.
- FAQ: 3–5 пар. Вопрос — живая человеческая формулировка, сохраняющая интент фразы;
  саму фразу НЕ вставляй дословно и не склеивай слова в порядке поискового запроса
  («Где найти achers одежда новосибирск?» — так нельзя, это переспам; правильно:
  «Что за бренд Achers и откуда он?»). Отвечай ТОЛЬКО по фактам. Если для фразы в
  фактах нет ответа — пропусти её. Ответ 2–4 предложения, первое — прямой
  самодостаточный ответ.
- meta_description обязана называть хотя бы один конкретный бренд из ФАКТОВ — иначе
  описания городов получаются взаимозаменяемыми.
- FAQ: три пары, минимум две. Одна пара — это провал задачи, а не «мало фактов»:
  вопросы всегда можно построить из ФАКТОВ, даже если поисковых фраз не дали.
  Но не более ОДНОГО вопроса общего вида про город целиком — остальные обязаны
  называть конкретный бренд и его деталь (что шьёт, из чего, для кого, чем отличается).
  Общие вопросы одинаковы у всех городов, и такой FAQ отбраковывается как дубль.
- h1 — строго вида «Бренды одежды <город в родительном падеже>» (Перми, Новосибирска,
  Челябинска, Казани), без предлога «в городе». ≤ 70 символов; meta_title ≤ 70,
  meta_description ≤ 160.
- Не пиши абзацы-заполнители («это важная часть локальной индустрии», «каждый проект
  имеет своё видение моды»). Нет фактов на четвёртый абзац — пиши три.

Формат ответа — ТОЛЬКО валидный JSON без markdown:
{"h1": "...", "meta_title": "...", "meta_description": "...", "intro": "<p>...</p>", "faq": [{"question": "...", "answer": "..."}]}
EOT;

        $response = $this->generate($prompt, local: true, think: false, timeout: 300);
        $decoded  = $this->extractJson($response);
        ...
        return [
            'h1'               => $str($decoded['h1'] ?? null, 255),
            'meta_title'       => $str($decoded['meta_title'] ?? null, 255),
            'meta_description' => $str($decoded['meta_description'] ?? null, 500),
            'intro'            => $str($decoded['intro'] ?? null, 20000),
            'faq'              => array_slice($faq, 0, 5),
        ];
    }

    /**
     * SEO/GEO: текст стилевого хаба /{_locale}/style/{slug} — аналог generateCityHub,
     * но с урезанной моделью данных: шаблон (tailwind/style.html.twig) рендерит только
     * BrandStyle::description БЕЗ |raw, поэтому ответ — плоский текст без HTML-тегов,
     * без meta/FAQ (их у стилей пока нет — не выдумываем поля).
     *
     * Зачем grounded: единственные факты о стиле — состав каталога (бренды, их города,
     * anons). Придуманная «история стиля» — вода, за которую и держат страницу без
     * контента на позиции 5+ вместо топа (см. постановку задачи).
     *
     * @param string[] $phrases поисковые фразы по этому стилю, по убыванию спроса
     * @return array{description: ?string}
     */
    public function generateStyleHub(string $styleTitle, string $facts, array $phrases): array
    {
        // Тот же приём, что в generateCityHub: у стилей с тонким спросом (см. постановку
        // задачи — большинство стилей вообще без фраз) жидкий список фраз хуже, чем
        // просто факты каталога — модель начинает подгонять текст под шум.
        $phrasesList = count($phrases) < 3
            ? '(фраз мало или нет — НЕ используй их и не подгоняй текст под них; опирайся только на состав каталога из ФАКТОВ)'
            : implode("\n", array_map(static fn(string $p) => "- {$p}", $phrases));

        $prompt = <<<EOT
Стиль одежды: {$styleTitle}

ФАКТЫ из каталога WEARBASE (единственный источник истины):
{$facts}

Поисковые фразы реальных пользователей по этому стилю (по убыванию спроса):
{$phrasesList}

Составь текст посадочной страницы «бренды одежды в стиле {$styleTitle}» в каталоге
локальных российских брендов WEARBASE.

ЖЁСТКИЕ ПРАВИЛА:
- Опирайся ТОЛЬКО на ФАКТЫ выше. Не выдумывай историю стиля, даты его появления,
  моду, цены и статистику.
- Число брендов стиля (сколько их в каталоге, «более N марок») не упоминай нигде в
  тексте — его отдельно выводит страница из реальных данных каталога, и текстовое
  число тут же разойдётся с ним при следующем изменении каталога.
- Назови минимум 3 и максимум 6 конкретных брендов из ФАКТОВ; у каждого — один
  проверяемый факт (что шьёт, из чего, для кого — деталь из его anons/описания).
  Это единственное, что реально различает стили, — общие фразы про моду не различают.
- Первое предложение — самодостаточный ответ на «что такое стиль {$styleTitle} и
  какие российские бренды в нём работают». Понятно без заголовка.
- Текст: 2–3 абзаца, 600–1100 символов, ОБЫЧНЫЙ ТЕКСТ БЕЗ HTML-тегов и без markdown —
  шаблон страницы выводит его как есть, без разметки. Абзацы раздели одной пустой
  строкой (два переноса).
- Средняя длина предложения — до 15 слов, чередуй короткие и длинные; ни одного
  предложения длиннее 25 слов (длинные монотонные фразы читаются как ИИ-текст).
- Длинное тире «—» — не больше одного на абзац (частый маркер ИИ-письма).
- В каждом абзаце — минимум один проверяемый факт из ФАКТОВ (название бренда,
  деталь из anons/описания), не только общие слова.
- Никаких рекламных штампов («широкий ассортимент», «мир моды», «идеальный выбор»,
  «на любой вкус», «свой уникальный стиль», «своя специфика»).
- Не пиши про покупку напрямую без наценки маркетплейсов, про сайты и соцсети
  брендов — этот блок уже есть в шаблоне страницы.
- НИКОГДА не пиши о самих поисковых запросах и о пользователях сервиса. Фразы даны,
  чтобы подобрать лексику текста, а не как тема для рассказа. Формулировки вида
  «пользователи часто ищут…», «популярный запрос касается…» запрещены полностью.
- Каталог WEARBASE — про ЛОКАЛЬНЫЕ РОССИЙСКИЕ марки. Не пиши «российские и зарубежные»,
  «мировые бренды» и подобное, даже если у бренда импортные ткани или иностранное имя.
- Названия брендов переписывай из ФАКТОВ символ в символ. Не сокращай и не дописывай.
- Пиши только по-русски (латиница допустима лишь в названиях брендов). Ни одного
  символа других алфавитов.
- Поисковые фразы вплетай естественно и НЕ дословно: перестраивай порядок слов под
  живую речь. Одну фразу — максимум один раз на весь текст.
- Не пиши абзацы-заполнители («это важная часть индустрии», «каждый бренд имеет своё
  видение стиля»). Нет фактов на третий абзац — пиши два.

Формат ответа — ТОЛЬКО сам текст описания. Без markdown, без обёртки ```, без
заголовка и без кавычек вокруг всего текста.
EOT;

        $response = trim($this->generate($prompt, local: true, think: false, timeout: 300));
        // Модель иногда всё равно оборачивает ответ в ``` или вставляет <p> — снимаем,
        // как в generateListicle/generateCityHub. </p> заменяем на двойной перенос,
        // чтобы не склеить абзацы в одно предложение при удалении тегов.
        $response = preg_replace('/^```\w*\s*|\s*```$/u', '', $response) ?? $response;
        $response = preg_replace('/<\/p>/iu', "\n\n", $response) ?? $response;
        $response = trim(strip_tags($response));
        $response = trim(preg_replace('/\n{3,}/u', "\n\n", $response) ?? $response);

        return ['description' => $response === '' ? null : mb_substr($response, 0, 20000)];
    }

    /**
     * SEO/GEO: текст хаба аудитории /{_locale}/audience/{slug} (Женщины/Мужчины/Дети/
     * Унисекс) — аналог generateStyleHub, тот же плоский формат без HTML/meta/FAQ:
     * шаблон (tailwind/audience.html.twig) рендерит BrandAudience::description БЕЗ |raw.
     *
     * $topicPhrase — H1-формулировка страницы («российские бренды женской одежды»,
     * дословно из Wordstat, см. BrandAudience::h1) — задаёт тему прозы; $audienceTitle
     * («Женщины») — только для человеко-читаемых ссылок внутри правил.
     *
     * @param string[] $phrases поисковые фразы по этой аудитории, по убыванию спроса
     * @return array{description: ?string}
     */
    public function generateAudienceHub(string $audienceTitle, string $topicPhrase, string $facts, array $phrases): array
    {
        // Тот же приём, что в generateStyleHub: тонкий список фраз хуже, чем просто
        // факты каталога, — модель начинает подгонять текст под шум.
        $phrasesList = count($phrases) < 3
            ? '(фраз мало или нет — НЕ используй их и не подгоняй текст под них; опирайся только на состав каталога из ФАКТОВ)'
            : implode("\n", array_map(static fn(string $p) => "- {$p}", $phrases));

        $prompt = <<<EOT
Аудитория каталога: {$audienceTitle}
Тема посадочной страницы: «{$topicPhrase}»

ФАКТЫ из каталога WEARBASE (единственный источник истины):
{$facts}

Поисковые фразы реальных пользователей по этой теме (по убыванию спроса):
{$phrasesList}

Составь текст посадочной страницы «{$topicPhrase}» в каталоге локальных российских
брендов WEARBASE.

ЖЁСТКИЕ ПРАВИЛА:
- Опирайся ТОЛЬКО на ФАКТЫ выше. Не выдумывай историю рынка, даты, моду, цены и статистику.
- Число брендов (сколько их в каталоге, «более N марок») не упоминай нигде в тексте — его
  отдельно выводит страница из реальных данных каталога, и текстовое число тут же
  разойдётся с ним при следующем изменении каталога.
- Назови минимум 3 и максимум 6 конкретных брендов из ФАКТОВ; у каждого — один
  проверяемый факт (что шьёт, из чего, для кого — деталь из его anons/описания).
  Это единственное, что реально различает бренды внутри темы, — общие фразы про моду
  не различают.
- Первое предложение — самодостаточный ответ на вопрос «{$topicPhrase}»: какие
  российские бренды сюда входят. Понятно без заголовка.
- Текст: 2–3 абзаца, 600–1100 символов, ОБЫЧНЫЙ ТЕКСТ БЕЗ HTML-тегов и без markdown —
  шаблон страницы выводит его как есть, без разметки. Абзацы раздели одной пустой
  строкой (два переноса).
- Средняя длина предложения — до 15 слов, чередуй короткие и длинные; ни одного
...
- Поисковые фразы вплетай естественно и НЕ дословно: перестраивай порядок слов под
  живую речь. Одну фразу — максимум один раз на весь текст.
- Не пиши абзацы-заполнители («это важная часть индустрии», «каждый бренд имеет своё
  видение»). Нет фактов на третий абзац — пиши два.
...
EOT;
        ...
        $response = trim($this->generate($prompt, local: true, think: false, timeout: 300));
        // Тот же приём снятия обёртки, что в generateStyleHub/generateCityHub.
        $response = preg_replace('/^```\w*\s*|\s*```$/u', '', $response) ?? $response;
        $response = preg_replace('/<\/p>/iu', "\n\n", $response) ?? $response;
        $response = trim(strip_tags($response));
        $response = trim(preg_replace('/\n{3,}/u', "\n\n", $response) ?? $response);
...
EOT;
     ...
     * @param string|null $anchorFacts факты О САМОМ {$anchorName} (описание/RAG) — ТОЛЬКО для
     *        проверки факта о переименовании его точек в России (rename-хук); без переименования
     *        в фактах хук не всплывает — LLM его просто не пишет.
    ...
    public function generateReplacementListicle(
        ...
        // Rename-хук (только если факт есть — иначе блок не добавляем и LLM неоткуда его взять).
        $renameBlock = '';
        $renameRule  = '';
        if ($anchorFacts !== null && trim($anchorFacts) !== '') {
            $renameBlock = "\n\nФАКТЫ О {$anchorName} (используй ТОЛЬКО чтобы проверить, есть ли факт о переименовании его магазинов/точек в России — не описывай сам бренд подробно):\n" . trim($anchorFacts);
            $renameRule  = " Если в блоке «ФАКТЫ О {$anchorName}» ЕСТЬ факт, что его магазины сейчас работают под другим названием — упомяни это одной нейтральной фразой во вступлении; если такого факта там нет — не упоминай переименование вообще.";
        }

        // Живой голос Дзена (item 2): псевдодиалог-зачин, короткие абзацы, эмодзи, вопрос в конце.
        // Конфликт — из ситуации (уход/дефицит), не из критики брендов.
        $dzenVoiceRule = $noTables
            ? "\n- Открой текст короткой репликой от лица кураторки, например: «Вы часто спрашиваете, чем заменить {$anchorName}, — я собрала то, что стоит посмотреть», и дальше веди от первого лица.\n"
            . "- Короткие абзацы (2–4 строки), уместные эмодзи (не в каждом абзаце, без перегруза).\n"
            . "- Заверши текст вопросом к читателю для комментариев, например: «А какой бренд вы для себя нашли на замену?».\n"
            . "- Конфликт подавай через ситуацию (уход брендов из России, меньше выбора на полке), а НЕ через критику или принижение {$anchorName} или брендов-замен."
            : '';

        $systemPrompt = "Ты — {$persona}, женщина. Пишешь обзорную статью на русском языке от лица кураторки WEARBASE. "
            . 'Если пишешь от первого лица — используй ТОЛЬКО женский род (собрала, выбрала, посмотрела, зашла), никогда не мужской. '
            . "Тон и формат: {$platformTone}. "
            . "{$anchorName} — иностранный бренд, ограничивший работу в России; упоминай его нейтрально, "
            . 'как точку отсчёта (nominative use). ЗАПРЕЩЕНО: оценочные и эквивалентные сравнения с '
            . $anchorName . ' («лучше», «хуже», «не хуже», «на уровне ' . $anchorName . '»), '
            ...
            . 'РАЗРЕШЕНО и НУЖНО: нейтральный функциональный мэтч — для каждого бренда одним предложением сказать, '
            . 'какую потребность он закрывает (сегмент/тип одежды/для кого), без оценок «лучше/хуже/как ' . $anchorName . '». '
...
{$brandsBlock}{$renameBlock}

Требования:
- ОБЪЁМ: 1300–2000 слов (это важно для SEO). Раскрывай каждый бренд подробно.
- НАЧНИ с блока «## Коротко»: 40–60 слов, прямой самодостаточный ответ на запрос «чем заменить {$anchorName} и как выбрать» по состоянию на {$year} год, понятный БЕЗ чтения остального — для AI Overview/сниппета. Упомяни {$anchorName} по имени.
{$comparisonRule}
- Вступление: 2–3 абзаца — {$anchorName} ограничил работу в России (нейтрально, без оценок); по каким критериям отобраны бренды-замены (ассортимент, ценовой сегмент, где купить); как выбирали.{$renameRule}
- Для каждого бренда секция: заголовок «## {номер}. {название}», затем 3–4 абзаца СТРОГО по его фактам (ассортимент, особенности, материалы, для кого, ценовой сегмент — если есть в фактах), и одно нейтральное предложение — какую потребность из круга {$anchorName} закрывает именно этот бренд (сегмент/тип одежды/для кого), БЕЗ оценок «лучше/хуже/на уровне {$anchorName}».
- Не выдумывай цены, даты, цифры и сравнения, которых нет в блоке бренда. Разрешено только нейтральное функциональное сравнение (какую нишу/потребность закрывает бренд) — БЕЗ оценочных и эквивалентных сравнений с {$anchorName}.
