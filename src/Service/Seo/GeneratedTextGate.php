...
class GeneratedTextGate
{
    ...
    private const JUNK_PATTERNS = [
        'пересказ поисковых запросов' => '/поисковы[ех]\s+запрос|популярн\w+\s+запрос|пользовател\w+\s+(часто\s+)?ищ|многие\s+ищ\w*|добавля\w+\s+к\s+запрос|поиск\s+\S+\s+.{0,40}приводит|запрос\w*\s+(касается|звучит)/iu',
