...
| Хост | Что там | Доступ |
|---|---|---|
| LLM-сервер / майнинг-риг | ollama :11434, Qdrant :6333, SearXNG :8080, wildrig, дашборд :8088 | `ssh llm` (сейчас 192.168.2.43; **IP непостоянен** — при недоступности сканировать подсеть на порт 11434 и обновить `~/.ssh/config` + память) |
...
- 🛑 CPU LLM-сервера слабый (i7-2670QM, 4C/8T) — расчёты LLM на CPU не пускаем: модель целиком в VRAM (`ollama ps` → `100% GPU`, в journal `offloaded N/N layers`), без CPU-offload слоёв и `--n-cpu-moe`. Не влезает — другая раскладка/модель, а не спилл на CPU.
...
3. **Авто-коммит + ветка на правку**: после каждой законченной правки — атомарный коммит без напоминания. `main` защищён (прямой пуш запрещён) — работать через `<type>/<desc>` ветку + PR. Мерж PR в main **автоматически деплоит на прод** (GitHub Actions, после зелёных тестов). Ручной `/deploy` (rsync с Mac) — независимый запасной путь. Детали — [[wearbase-cicd-github-actions]] / см. раздел «Прод и деплой».
...
**CI/CD (с 2026-07-26):** `main` под branch protection (мерж только через PR). Workflow `.github/workflows/ci.yml`: PHPUnit на каждом PR → на мерже в main автодеплой на прод (rsync из облака + миграции + cache:clear + smoke). Секреты деплоя — в GitHub Secrets (`DEPLOY_*`). Скилл `/deploy` (ручной rsync с Mac) остаётся как запасной/срочный путь. Полная схема и грабли CI — docs README / память `wearbase-cicd-github-actions`. ⚠️ Всегда коммить `composer.lock` после `composer update` — иначе CI (ставит по локу) разъедется с прод-vendor.
