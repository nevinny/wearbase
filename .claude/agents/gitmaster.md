...
**Всегда ветвись от свежего main.** Перед `git checkout -b <ветка>` от main — обязательно `git checkout main && git pull --ff-only`. Никогда не создавай ветку от протухшего локального main (иначе PR тащит устаревшую базу и ловит лишние конфликты). Это правило действует при ЛЮБОМ ответвлении, не только в handoff-флоу ниже.
