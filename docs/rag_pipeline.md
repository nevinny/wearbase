...
11. **`app:brand:crawl` берёт own_site через `findOneBy` — без сортировки.** Если discover положил
    несколько own_site (частый случай: B2B-каталоги productcenter.ru / fabricators.ru получают
    own_site t1 relevance 1.00), краул может развернуть sitemap **каталога**, а не сайта бренда, и
    залить корпус мусорными рубриками («очки», «заборы», «ЖКХ»). Симптом: 30 own_page с чужого
    хоста. Лечение: понизить каталоги до `mention` (`UPDATE brand_source_url SET
    source_type='mention' …`), soft-delete мусорные документы, перезапустить `crawl --id=`.
12. **SPA-сайты дают 200 и 0 символов.** Клиентский рендер (весь контент в `window.__dkInit` и
    подобных JSON-блобах) → trafilatura/DomCrawler извлекают пустоту, fetch помечает URL
    `fetched`, но документов не создаёт («Пусто/дубль» на все страницы). Пример: salon-dashenka.ru
    (бренд Дашенька, #7530) — 27 страниц, 0 документов. Обход: вытащить строки из JSON-блоба и
    вставить факт-текст документом вручную (§9), либо жить на профилях бренда в каталогах.
...
### Топология: кто где (prod + Mac + LLM-сервер)

```
прод (витрина) → индекс Яндекса/Google → API Вебмастера/GSC
      ↑                                        ↓
агент-API push ← LLM-сервер (реген) ← Mac (замер + вердикт)
```

- **Прод (regru) — «глупая» витрина.** Отдаёт страницы, которые краулят ПС, и принимает доставки
  через агент-API (`/api/v1/brands/upsert`, HMAC). Поисковых данных и логики оценки на проде НЕТ
  (его `gsc_index_status` пуст by design); единственный входящий сигнал — `drip_health` для дрипа.
- **Mac — мозг цикла.** Все данные и решения здесь: `app:yandex:sync` (07:00) и `app:gsc:sync`
  (08:00) стягивают метрики ПС в Mac-БД (`yandex_*`, `gsc_*`); ревизии (`brand_content_revision`)
  живут в Mac-БД; `evaluate-experiments` (10:00) выносит вердикты по локальным данным. Гард
  свежести обоих синков: Mac выключен → синки протухли → вердикты не выносятся (пауза, не ошибка).
- **LLM-сервер (.111) — руки регена.** Loss-ветка ставит `regen_requested_at` (Mac-БД) →
  подхватывает generate-стадия RAG-демона на LLM-сервере (`PipelineQueueRepository`). Резервный
  Mac-крон ночного регена (03:00) **выключен** (enabled=0) — чтобы не переподписывать GPU демону.
- **Доставка на прод.** И реген, и откат заканчиваются пометкой `content_changed_at` → сетевая
  стадия `app:brand:push` (демон) доставляет бренд на прод через агент-API. ПС переиндексируют
  страницу → следующий синк это видит → круг замкнулся.
- **Асимметрия намеренная:** интеллект стянут на Mac, прод дёшев и заменяем. Следствие: цикл жив
  настолько, насколько жив Mac; выключенный Mac = безопасная пауза всего контура (дрип на проде
  продолжает работать сам, fail-open).
...
Контур самокрутится:
- `0 7` — `yandex:sync --report` (замер: страницы в поиске Яндекса + запросы + история; первичный источник с 03.07);
- `0 8` — `gsc:sync --report` (замер: показы/индекс Google + sitemaps; вторичный — покрытие заморожено с ~12.06);
...
- реген проигравших — **generate-стадия RAG-демона на LLM-сервере** (флаг `regen_requested_at` в очереди).
  Резервный Mac-крон `0 3` (`generate-content 50 --regen-flagged --protect-performing`) **выключен**
  (enabled=0) — включать только при выключенном демоне (иначе переподписка GPU).
