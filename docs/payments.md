...
**Не покрыто автотестами** (харнес `WebTestCase` сломан — конфликт двух User-сущностей):
рендеринг страниц ЛК/лендинга, формы (импорт/переводы/оферта/счёт), реальный вебхук-путь заказа.
Проверяется `lint:twig` + `lint:container` + ручной смоук. Все live-money-пути требуют ручной проверки перед продом.
