...
## 2026-06-13 — Виртуальная примерочная (VTON): PoC + документация

### Added
- **`docs/virtual-tryon.md`** — полное руководство по PoC виртуальной примерочной и план запуска в прод:
  двухстадийный конвейер (мастер-фото с сохранением лица → примерка CatVTON), стек на .43,
  agnostic-маска по зоне тела, известные ограничения, план интеграции в wearbase (сущности,
  Symfony Messenger, ПД/согласие, маска по категории товара). PoC-скрипты — `~/tools/vton-poc/`.

### Notes
- PoC доказан на .43 (A4000): верх — продакшен-уровень, низ/юбки — рабочий силуэт. Прод-интеграция
  ещё не сделана. v3 (фотореализм низа) требует IDM-VTON/Leffa + DensePose на машине с AVX-CPU.
...
- **Публикация блога**: `app:blog:publish-drafts` — идемпотентная публикация статей из `docs/blog-drafts/*.html` по манифесту (4 статьи; контент синхронизирован с локальной БД). Чинит 404 на `/ru/blog/komissii-marketpleysov-2026` на проде: статьи были опубликованы только локально.
