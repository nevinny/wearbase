...
| `app:family:purchase-reminders` | Раз в локальный день: in-app для pending/delivered; после `bought` — in-app и включённые external-каналы в окне 7–14 дней и с 30-го дня, без автоматической носки. `--dry-run`, `--now=ISO-8601`; граница — Europe/Moscow, дедуп по этапу/позиции/получателю. | ⏰ `15 9 * * *` | ☁️ prod |
