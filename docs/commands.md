...
⚠️ **Длинный джоб съедает слоты соседей — и слот пропадал навсегда** (разбор 2026-08-01).
Лок глобальный, а джоб запускается синхронно внутри тика: пока он идёт, каждый тик печатает
«Пропуск: предыдущий проход ещё выполняется». `isDue()` совпадает **только в свою минуту**,
поэтому задача, чья минута попала в дыру, не запускалась НИКОГДА — `app:gsc:sync` держал
08:00–09:00, и `app:advisor:snapshot` (50 8 * * *) молчал 13 дней, `app:social:evaluate`
(0 9 * * 1) — с 13 июля, `app:kb:sync-tg` (30 8 * * *) не отработал ни разу. Теперь диспетчер
**догоняет пропущенный слот** по `next_run_at` (у задач без истории `next_run_at IS NULL` —
догонять нечего, они ждут своей минуты). Отсюда два правила: у каждой задачи в кроне должен
быть внутренний бюджет времени заведомо меньше её `timeout_sec`, а диагностика «почему задача
молчит» начинается с `grep 'Пропуск: предыдущий проход' var/log/cron.log`, а не с самой задачи.
...
| `app:seo:gap-report` | Автопилот position-листа (docs/yandex_ai_visibility_monitoring.md) — раньше два ручных SQL по пятницам. `yandex_query_stats` (последний `date_to`) + `gsc_query_stats` (сумма по окну), группировка по интенту тем же `AioQueryClassifier`, что `app:seo:aio-queries`: brand_entity / replace_comparison / geo_category / navigation (совпал title/slug опубликованного бренда) / other. **Две полосы позиций `--band`:** `striking` (3<pos≤10 — первая страница без топ-3, дожать существующий URL) и `gap` (pos>10 — вторая страница); не пересекаются, striking идёт первым. URL-владелец резолвится для ОБОИХ источников: GSC — из `gsc_query_page`, Яндекс — из `yandex_query_page` (см. `app:yandex:sync`). Пишет снапшот `seo_gap_snapshot` (по `source`+`band`+`intent_group`), не мутирует бренды. `--notify` — компактная сводка в TG тем же `AdminNotifier`, что `app:seo:aio-remediate` — **отдельный крон, не вклеено в report:daily/weekly**. `--source=yandex\|gsc\|both`, `--min-shows`, `--limit`, `--stdout-only`, `--json`. | ⏰ `0 8 * * 1` (пн, до aio-remediate 8:40) | 🍎 Mac |
| `app:seo:tech-audit` | Тех-аудит обходом живого сайта (docs/seo_guide_vasin_gap.md, пробелы 2–3): мета/H1/canonical/alt/дубли title, CTA, оформленный заголовком H1–H4, блок FAQ без JSON-LD `FAQPage`, заглавные/кириллица в URL, битые внутренние ссылки, **страницы-сироты** (в sitemap, но без входящих внутренних ссылок). Обход в две фазы: сначала хабы до исчерпания очереди (`--hub-limit`), потом карточки брендов остатком `--limit`; сироты считаются ТОЛЬКО при исчерпанной фазе хабов, иначе проверка честно пропускается. noindex-страницы (корзина/ЛК/авторизация) из мета-правил исключены. Findings ведутся в `seo_tech_finding` с `fixed_on` (soft-delete, без DELETE) — в TG уходит только **дельта «появилось / исправлено»**. `--base` (по умолчанию `app.site_base_url`), `--link-check-cap`, `--delay-ms`, `--stdout-only`, `--json`. | ⏰ `30 7 * * 6` (сб) | 🍎 Mac |
...
| Команда | Зачем | Как часто | Где |
|---|---|---|---|
| `app:gsc:sync` | Search Analytics **тремя срезами**: `[page,date]` → `gsc_page_stats`, `[query,date]` → `gsc_query_stats`, `[query,page]` за окно → `gsc_query_page` (URL-владелец запроса для дожима позиций). Срезы НЕ сводимы один в другой: замер 2026-07-28 — сведение query×page в page-агрегат теряет 50.7% показов (GSC при группировке по query отбрасывает данные), а в query-агрегат раздувает показы на каннибализированных запросах. Экономить вызовы не от чего: 1200 QPM на сайт против 3 запросов в сутки. Пагинация `startRow` — чтобы усечение на `rowLimit=25000` не было молчаливым. Плюс URL Inspection (cap 1500/день, приоритет свежим) → `gsc_index_status`. Fail-open. | ⏰ `0 8 * * *` | 🍎 Mac |
| `app:yandex:sync` | RU-аналог `app:gsc:sync`. `search-urls/in-search` → `yandex_index_status`; `search-queries/popular` **с пагинацией** → `yandex_query_stats` (показы/клики/**позиция**; раньше брали жёстко 500 при 2040 в выдаче — видели четверть); `query-analytics/list` (POST) → `yandex_query_page` — единственный у Яндекса разрез «запрос → наш URL» (позиции в нём нет, связка с `yandex_query_stats` по тексту запроса); `summary` + `diagnostics` + `links/internal/broken` → `yandex_site_health` (ИКС, страниц в поиске, исключено, активные проблемы) с алертом в TG на severity FATAL/CRITICAL/ERROR — там же живут **нарушения (МПК)**. ⚠️ «Битые» ссылки Вебмастера пишутся в `seo_tech_finding` (правило `yandex_broken_link`) только после подтверждения HTTP-кодом: замер 2026-07-28 — 74 сообщённых пары, подтверждено 0 (в этот раздел попадает и «недоступное роботу»: `/cart`, `/login` отдают 200, `/account/orders` → 302). `--queries-cap`, `--no-health`, `--urls-only`, `--queries-only`, `--report`. Fail-open. | ⏰ `0 7 * * *` | 🍎 Mac |
...
| `app:seo:listicle` | **SEO Boost / GEO**: статья-рейтинг «ТОП-N в нише» с целевым брендом №1 + реальные конкуренты той же ниши; grounded (описание+RAG), JSON-LD Article+ItemList+FAQPage, quality-gate перед сохранением (бренд не пропущен/отказ/штампы). По умолчанию пишет в `var/seo/`. `<brand_id> [niche]`, `--city` (гео-срез «ТОП {стиль} {город}»), `--top --platform`(vc/dtf/pikabu/press/blog/**dzen**)` --persona --variants --no-faq --force --out`, `--gap-context=<id>` (темы конкурентов из `app:seo:competitor-scan`). Свой аналог КП ContentMagic. Полный reference — [seo_boost.md](seo_boost.md). | 👆 по запросу | 🖥/🍎 (нужна локальная LLM) |
| `app:seo:competitor-scan` | **Разведка конкурентов в выдаче** (Skyscraper, docs/seo_competitor_content.md): по популярным SEO-фразам (`SeoQueryGapProvider` — та же логика, что `app:seo:gap-report`, + опционально `--source=wordstat`) смотрит живую выдачу (`YandexSearchClient`, платный API), классифицирует URL (article/marketplace/social/official_brand_site), скрейпит только article-результаты (30-дневный кэш `competitor_article`), извлекает у них ТОЛЬКО темы/структуру (не факты — юридический риск пересказа) и печатает рекомендованный роут на `app:seo:listicle`/`app:seo:replace-listicle`/`app:seo:guide --gap-context=<seo_competitor_scan id>`. `--source=wordstat\|gsc\|yandex\|both`, `--intent=geo_category,replace_comparison,other`, `--limit`, `--serp-limit`, `--notify`, `--dry-run`. | 👆 по запросу (платный API, в крон не добавлена) | 🖥/🍎 (нужна локальная LLM + Yandex Search API ключ) |
