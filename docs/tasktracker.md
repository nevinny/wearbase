...
- [ ] **Platron** — агрегатор (карты, СБП/QR, SberPay, T-Pay, Mir Pay, Yandex Pay). Заявка 2026-09-07, ответили стандартным инфозапросом СБ. Открытые вопросы к провайдеру: (1) подключают ли получателя на НПД (самозанятый, ИНН 771910731248) или только ИП/ООО; (2) поддержка расчётов по маркетплейс-схеме (сплит на юрлица продавцов) — нам деньги за заказ на счёт площадки не нужны. Тарифы/минимальный оборот на сайте не публикуются — только по запросу.
  - **Ответ 07.09.2026: только ИП/ООО** (самозанятого не подключают). Ставки для сервиса с подпиской: карта и *Pay — **4,85% + 20 ₽/транзакция**, СБП — **2,5% + 20 ₽**. На наших чеках это 5,2–6,2% (карта) и 2,9–3,8% (СБП) — фикс 20 ₽ бьёт по чекам 1 500–5 000 ₽.
  - **Решение: пас.** ЮKassa подключает самозанятых (кошелёк ЮMoney), карта от 2,8%, СБП от 0,4–0,7%, фикса за транзакцию нет → дешевле и без открытия ИП. Наш `YooKassaGateway` уже готов, ключи в .env.local сейчас `test_` — боевого договора нет, это и есть следующий шаг.
  - ИП/ООО открывать по бизнес-триггеру (лимит НПД 2,4 млн/год · приём оплаты заказов и сплит · 289-ФЗ с 01.10.2026), а не ради ставок Platron: постоянные расходы ИП ≈ 80–100 тыс ₽/год (взносы + облачная ККТ по 54-ФЗ; самозанятый бьёт чек в «Мой налог» бесплатно).
  - **Условия Platron (переписка 07.09.2026, зафиксировано):**
    - Подключают **только ИП/ООО**, самозанятого — нет.
    - **Подписка/услуги площадки:** карта и *Pay-методы — **4,85% + 20 ₽/транзакция**; СБП — **2,5% + 20 ₽/транзакция**.
    - **Фикс 20 ₽ банк пересматривает только при обороте от 1 млн ₽/мес** (наш прогноз 30–150 тыс ₽/мес → нерелевантно надолго).
    - **Оплата товаров** — отдельная ставка: ≈ **1,2% СБП / 3,5% карты**.
    - **Продукта «маркетплейс» (сплит на юрлица продавцов) у Platron нет.** Значит оплата товаров = приём на счёт мерчанта; если мерчант — площадка, это ломает принцип «деньги за заказ напрямую бренду» (payments.md §1) и тянет платёжно-агентские риски. Как эквайрер **самого бренда** (бренд-мерчант, свои реквизиты в `SellerPaymentAccount`) ставка нормальная.
    - Методы: карты, СБП/QR, SberPay, T-Pay, Mir Pay, Yandex Pay, ЮMoney. Тарифы публично не опубликованы — только по запросу; далее их процесс = отправка сайта на проверку в банки.
    - **Итог: для площадки пас.** Открытый вопрос к ним: есть ли агентская/реферальная программа для площадки, приводящей мерчантов (наши бренды — ИП/ООО), и как быстро проходит подключение мелкого ИП. Если да → Platron осмыслен как ещё один провайдер в пикере ЛК бренда, а не как эквайринг WEARBASE.
    - Второй этап (оплата заказов покупателей) требует сплит-продукта → смотреть ЮKassa Сплит / Best2Pay, не Platron.
...
## 2026-09-11 — Исследование спроса брендов и первый cold-батч нового оффера

Прогнали метод McKinsey/NotebookLM (разбор видео → [market_research_notebooklm_method.md](market_research_notebooklm_method.md))
по объекту «бренды как клиенты». Полный отчёт — [brand_demand_research_2026-09.md](brand_demand_research_2026-09.md).

### Находка

Спроса «бренду нужна витрина» не существует: «где продавать одежду кроме вайлдберриз» — **1
запрос/мес**, «маркетплейс для дизайнеров одежды» — 6. При этом «российские бренды одежды» —
**21 056/мес**, и этот спрос уже приходит к нам: 55 501 показ и 1134 клика на карточки 2137
брендов за 28 дней (GSC). Четыре месяца продавали площадку, которую никто не ищет, вместо того
чтобы предъявить бренду спрос на него самого.

| Метрика (прод, 11.09) | Значение |
|---|---|
| Брендов с ≥100 показов за 28 дней | 80 |
| Из них в ЛК | 0 |
| Из них получали аутрич | 2 |
| Из них без контакта вообще | 52 |
| Брендов с показами и 0 кликов | 29 (6018 показов вхолостую) |

### Сделано

- PR #223 — канал Дмитрия Попова в базу знаний советника (role `framing`, 30 чанков).
- PR #224 — отчёт исследования + адресный список `var/research/brand_demand_outreach_list.csv`
  (вне git — там email).
- PR #225 — cold-письмо приведено к `legal_email_outreach.md`: рекламный блок удалён, добавлен
  факт поискового спроса; факты передаются `--facts` JSON, т.к. GSC живёт на Mac, а письма
  уходят с прода.
- Отправлено 8 писем (16:03–16:06), 0 ошибок — [outreach_demand_experiment_2026-09.md](outreach_demand_experiment_2026-09.md).

### Грабли

- **id брендов на Mac и на проде разъехались** (id 2088: Mac `donna-dora`, прод `mima`). Склейка
  dev↔прод — только по `slug`. Первый счёт по id был неверным и пересчитан.
- Ключ Wordstat живой; `numPhrases` обязателен, иначе `code 3` — это не признак мёртвого ключа.
- DKIM-селектор по DNS не угадывается — проверять разбором доставленного письма на свой ящик.

### Открытое

- [ ] 52 бренда приоритетного списка без контакта — добыть контакты.
- [ ] DMARC `p=none` → `quarantine`; вебхук RuSender `delivered`/`bounce`.
- [ ] Бэкафилл `origin_status` — иностранные марки автогейт не ловит.
- [ ] 29 карточек с нулевым CTR — заголовки и сниппеты.
