...
### P0A — короткий UX AI-стилиста и коллекции

- [ ] Сделать AI-стилиста главным CTA: контекст → 1–3 образа → надеть/заменить/«не моё» без
  обязательного заполнения полной анкеты.
- [ ] Автоматически извлекать максимум данных из фото, товара и истории; спрашивать только поля,
  которые AI не уверен определить и которые влияют на текущий результат.
- [ ] Ввести конечную карточную сессию без autoplay/infinite scroll и измерить time-to-first-outfit.
- [ ] Добавить практические коллекции: капсулы, освоенные вещи, новые сочетания и сезонную карту;
  не награждать за покупку и не требовать искусственных 100% коллекции.
- [ ] Добавить полезные достижения за повторную носку, возврат вещи в ротацию, ремонт и готовую
  поездку; награда — insight/коллаж/образ, без виртуальной валюты.
- [ ] Реализовать добровольные семейные co-op задачи без публичного рейтинга внешности, стоимости
  гардероба или сравнения детей.
- [ ] Запретить dark patterns для несовершеннолетних: streak loss, FOMO/countdown, loot boxes,
  бесконечную ленту, наказание за пропуск и body/value leaderboards.
- [ ] Провести usability-тесты с подростками и родителями; A/B проверять короткий визуальный цикл,
  контекстные подсказки и коллекции по `saved/worn`, ручным действиям и guardrail-метрикам, а не
  только retention.

### P0B — onboarding и оцифровка существующего гардероба

- [ ] Спроектировать resumable onboarding «обучение действием»: выбор профиля → первая мини-капсула
  из 5–10 вещей → первый AI-образ → реакция; tutorial можно пропустить и открыть позднее.
- [ ] Показывать честное покрытие: сколько вещей известно стилисту и какие категории/сезоны ещё
  не оцифрованы; отсутствие вещи в базе не интерпретировать как необходимость покупки.
- [ ] Сделать три явных пути первого наполнения: пакетные фото, одна вещь и импорт подтверждённой
  покупки; все создают тот же `WardrobeItem`/draft через существующий application layer.
- [ ] Адаптировать пакетную загрузку под мобильную камеру: последовательная съёмка, фоновые upload
  и recognition, сохранение/возобновление batch после закрытия PWA и понятный offline retry.
- [ ] Разделить AI-review на `принять уверенные`, `проверить сомнительные`, `не распознано`; добавить
  bulk accept, undo, duplicate detection и возможность закончить сессию без обработки всего batch.
- [ ] Добавить конечные задания по месту/категории: «5 верхов», «эта полка», «обувь», «зимние вещи»;
  пользователь сам выбирает порядок, streak и обязательная полная инвентаризация отсутствуют.
- [ ] После фото образа создавать draft только для неизвестных вещей и сопоставлять известные;
  подтверждение пользователя обязательно до попадания в гардероб и статистику носок.
- [ ] После статуса покупки `bought` предлагать идемпотентное создание карточки с уже известными
  фото, ценой, размером и provenance без повторного ручного ввода.
- [ ] Добавить постоянный inbox качества данных: дубли, вещи без фото/категории, устаревшие размеры
  ребёнка; показывать только действия, влияющие на текущий сценарий стилиста.
- [ ] Вычислять мягкий прогресс по покрытию полезных категорий и сезонов, не по размеру коллекции;
  архивирование, передача и удаление ненужной вещи не должны снижать достижения.
- [ ] Сделать role-aware onboarding: ребёнок оцифровывает только себя, родитель может работать от
  имени managed-child с явными `actor/profileSubject`; прогресс и batch изолированы между профилями.
- [ ] Покрыть E2E: первый вход/skip/resume, мини-капсула→образ, batch background/retry, bulk review,
  duplicate/unknown item, bought→WardrobeItem и запрет cross-family/profile IDOR.
- [ ] Измерять activation как `first items → first outfit → worn`, время/ручные действия на одну
  принятую вещь, completion batch и рост разнообразия подтверждённых образов по мере покрытия.

### P0C — production foundation для onboarding

> Блокирует выпуск P0B. Реализуется отдельной feature-веткой/PR после стабилизации семейных покупок.

- [ ] Перенести wardrobe/draft-фото из public web root в private storage; отдавать через
  авторизованный media endpoint с `private,no-store`, проверкой actor→profileSubject и без EXIF.
- [ ] Разделить в ingest `actor` и `profileSubject`: child загружает только себе, parent — управляемому
  ребёнку после `FamilyService::resolveMember`; parent→spouse определяется отдельной capability.
- [ ] Изолировать review/status/counts одновременно по batch и profileSubject; одинаковый batch ID
  у разных профилей не должен смешивать счётчики или черновики.
- [ ] Вынести promotion draft в один application service: допустимый status, DB transaction/lock,
  terminal accepted state и связь с созданным item обеспечивают идемпотентный accept/retry.
- [ ] Добавить upload idempotency key/content hash, лимит количества/общего размера, max pixels,
  rate limit и понятный all-invalid ответ без пустого batch.
- [ ] Исправить worker: фильтр batch до limit, atomic claim/lease, attempt/nextAttemptAt, продолжение
  после ошибки одного draft и видимый retry/exhausted state; закрепить production schedule/health lag.
- [ ] Не хранить полный AI payload: только нормализованные поля и correlation ID; определить TTL и
  cleanup abandoned/rejected/failed drafts и приватных файлов.
- [ ] Для первого релиза честно зафиксировать online-only upload. Resume идёт по server batch после
  повторного открытия; Background Sync на iOS не является гарантией и не обещается в UI.
- [ ] Добавить `WardrobeOnboarding` на profileSubject: active batch, skipped/completed timestamps и
  resumable stage без хранения хрупкого wizard payload.
- [ ] Security/integration tests: guest/child/parent/sibling/foreign matrix, private media, batch
  collision, double accept, worker lease/retry, quota/image bomb, retention и no-store.
- [ ] Playwright: skip/resume, parent→managed-child batch, закрытие/возврат, bulk review, reconnect
  без дубля и переход `accepted items → first outfit → reaction`.
