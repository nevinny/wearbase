...
## 2026-08-08 — AI-стилист: личное обучение и семейный гардероб

**Цель:** персональный стилист учится только на гардеробе выбранного члена семьи; общая система
...
- [ ] Ввести `actor`, `profileSubject`, `consentGrantor`, `signalSource`, `wardrobeOwner`:
  родительское наблюдение маркируется отдельно и не выдаётся за подтверждённый вкус ребёнка.
- [ ] Добавить «Сохранить», замену вещи, причины отказа, историю образов и изменение реакции.
- [ ] Добавить экспорт и удаление персональной AI-истории.
- [ ] Применить миграцию и пройти живой E2E после включения домашнего сервера.

### P1 — embeddings без смешивания гардеробов

- [ ] Создать Qdrant-коллекцию `wardrobe_personal_memory_chunks` (1024/Cosine) и единственный
  scoped gateway с обязательным `profileSubjectId`; unscoped-поиск не должен существовать.
- [ ] После содержательной реакции создавать embedding и передавать Gemma top-N похожих случаев
  только текущего владельца; сравнивать долю «Я это надел» с базовым вариантом.
- [ ] Семейный запрос не объединяет профили автоматически; «семейный образ» — отдельный opt-in режим.

### P2 — добровольный общий опыт

- [ ] После «Я это надел» предложить: оставить личным / поделиться обезличенно / опубликовать.
- [ ] Сделать отдельный шлюз обезличивания и коллекцию `wardrobe_shared_experience_chunks` без
  фото, личности, размеров и ID личных вещей; opaque consent/experience ID обеспечивает отзыв.
- [ ] Разделить согласия на рекомендации сообществу, публичную публикацию и обучение весов;
  для managed/minor-профилей ввести отдельные строгие правила.
- [ ] Адаптировать общую идею к вещам получателя и собирать сигнал «Этот опыт помог».
- [ ] До публичной ленты добавить модерацию, жалобы и защиту от накрутки.

### P3 — контролируемое дообучение

- [ ] Только после тысяч подтверждённых примеров с отдельным согласием: очищенный датасет,
  train/validation/test, пакетный LoRA, A/B по `worn / показы` и откат. Онлайн-обучение кликами запрещено.

### Инфраструктура и блокеры

- Ноутбук — без Ollama и моделей. Домашний сервер: Gemma, отдельная vision-модель, embedder
  `qwen3-embedding:0.6b`, Qdrant и единая GPU-очередь.
- Локальные тесты планируется направить через `forgetborders.com/llmq.php` (полный URL/токен —
  только `.env.local`); до этого нужен remote/OpenAI-клиент или адаптер. Production получит
  отдельный endpoint и секреты.
- [ ] Домашний сервер выключен: нельзя пройти E2E, проверить имена моделей, скорость и устойчивость.
- [ ] До массового запуска нужны Symfony Messenger, сериализация тяжёлых GPU-задач и понятное UX-ожидание.
- [x] Удалить реальные Qdrant/relay-секреты из `docs/`, заменить placeholders.
- [ ] Перевыпустить раскрытые ранее Qdrant/relay-секреты: удаление из текущего текста не чистит Git history.
- [ ] Обновить `.env.local.example`: ноутбук без Ollama, локальные тесты через remote/adaptor;
  не предлагать `127.0.0.1` и установку локальной модели.
- [ ] Провести конкурентное исследование семейных гардеробов и измерять activation семьи,
...
## 2026-08-03 — Валидация ключевиков на соответствие нише (НЕ ВЛИТО, ветка ждёт)

**Повод:** импорт лидов и Wordstat подмешивают мусорные related-фразы («яндекс погода», «яндекс
игры», «you are being», голые генерик-слова) даже нишевым брендам. Они текли в `meta.keywords`
(опасно для Яндекса), FAQ, соцпосты (`DemandCaptionSource` берёт топ-1 фразу!) и агент-payload на
прод. Балл спроса дрипа спасал только фильтр «имя+коммерческий модификатор» в ORDER BY, но сами
фразы в БД грязные.

**Критерий (утверждён владельцем):** ключ классифицируется на нишу **топикально** — про
одежду/моду/косметику как ТЕМУ, независимо от привязки к конкретному бренду. Метод: **LLM (локальная
gemma) на каждую уникальную фразу**. Дефолт для неклассифицированного: `NULL` = **fail-open**
(проходит), потребители режут только явный `'off'`.

### Состояние: готово и закоммичено, но НЕ интегрировано
- Всё реализовано и протестировано на ветке **`feat/keyword-niche-validation` @ `b818cb0`**
  (`feat(brand): валидация ключевиков brand_keyword на соответствие нише`). **Апдейт 2026-09-09:**
  ветка запушена, открыт **PR #213**; в main пока НЕ влита. При возобновлении нужен **rebase на
  актуальный main** (см. дрейф ниже), а до мержа — решить пересечение с PR #196 (минус-слова
  на приёме): оба правят `BrandKeyword` и добавляют миграцию на `brand_keyword`.
- Полный прогон тестов на момент коммита — 336 OK; профильные 6/6 (парсер классификатора + гейт
  репозитория, включая регресс на протечку между брендами).

### Что в коммите b818cb0
- Миграция `Version20260727_brand_keyword_niche.php`: `brand_keyword += niche_status VARCHAR(12) NULL,
  niche_checked_at DATETIME NULL` + индекс, идемпотентно (guard через information_schema).
- Сущность `BrandKeyword`: поля + константы `NICHE_IN`/`NICHE_OFF` + `isOffNiche()`.
- Сервис `src/Service/Keyword/KeywordNicheClassifier.php`: `classifyBatch()`, чанки по 30 фраз, один
  LLM-вызов на чанк, строгий JSON-массив `[{"k","v"}]`, битый чанк не роняет батч, сервер недоступен →
  фраза остаётся NULL.
- Команда `app:brand:keyword-niche-check` (`src/Command/CheckKeywordNicheCommand.php`): шаг 1 —
  SQL-пропагация уже известных вердиктов на одинаковые NULL-фразы **без LLM**; шаг 2 — LLM на
  оставшиеся DISTINCT-фразы лимитом; bulk-UPDATE по фразе. Опции `--force`/`--dry-run`, дружит с
  `--no-debug`.
- Потребители режут `off` в одной точке — `BrandKeywordRepository::findByBrandRanked/findTopByBrand`
  (`(niche_status IS NULL OR != 'off')`), автоматически закрыло генерацию meta/FAQ/соцпост. Для
  агент-payload добавлен `findExportableByBrand()` (в `BrandPayloadAssembler` вместо голого `findBy`).
- Прод чистится **сам**: `BrandIngestService::replaceKeywords()` — семантика **replace** (проверено),
  off-фразы уйдут при следующей синхронизации бренда. Прод-миграция/правка ORDER BY НЕ нужны.
- Найдено и починено на ревью (агент пропустил): предикат `A OR B` в `andWhere` без скобок сливался с
  `k.brand = :brand` через приоритет `AND > OR` → тянул `'in'`-ключи **чужих** брендов. Обёрнут в
  скобки во всех трёх методах + добавлен регресс-тест на протечку. (Тесты агента это не поймали —
  один бренд в фикстуре. Тот же класс, что вывод от 2026-07-31: зелёные тесты ≠ корректность.)

### Что осталось (порядок)
- [ ] **Rebase `feat/keyword-niche-validation` на актуальный main.** ⚠️ Дрейф в
  `src/Repository/BrandKeywordRepository.php`: на main добавлен метод `existsPhrase()`, а два
  read-метода (`findByBrandRanked`/`findTopByBrand`) в main **без** ниша-гейта — при rebase
  переприменить обёрнутый в скобки предикат поверх новой версии, не потерять `existsPhrase`. Прочие
  файлы фичи новые — конфликтов не дадут.
- [ ] Push ветки + PR (мерж = автодеплой на прод; миграция накатится и на проде — колонка там не
  используется, безвредна).
- [ ] **После мержа — бэкафилл на Mac:** `php -d memory_limit=512M bin/console
  app:brand:keyword-niche-check 2000 --no-debug` (разметить накопленный мусор; данные разъедутся на
  прод обычной синхронизацией). Потом — в крон.
- [ ] Прогнать бэкафилл сперва на пробной пачке (`--dry-run` уже проверен на 20 фразах, gemma
  размечает корректно) и глянуть **долю off** — это заодно ответ на исходный вопрос про качество
  хвоста очереди публикаций.

## 2026-07-26 — Автоматическое код-ревью PR (сессия монитора, закрыта)

Монитор PR (cron в сессии Claude, каждые 30 минут с 16:52 до 23:21) читал диф каждого нового PR,
публиковал ревью комментарием и мержил те, что проходили планку. **Джоба выключена, сессия закрыта.**

**Планка автомержа** (зафиксирована владельцем, память `auto-merge-clean-prs`): мержу только при
ревью без 🔴 и 🟠, механика `gh pr merge --squash --auto --delete-branch`. Причина планки — мерж в
`main` запускает автодеплой на прод, то есть автомерж = автономный прод-деплой.

**Результат:** 4 PR отревьюено (#26, #41, #44, #46), 1 смержен автоматически (#44 → в ветку #41,
не в main → без деплоя). Автор починил почти всё до мержа; все три остальных PR влиты 27.07.

Закрыто автором (проверено по `main` 03.08): дубли default-гардеробов при импорте (`getOrCreateDefault`
вынесен до цикла), `--create-user` с временным паролем в stdout (фича убрана), падающий `down()`
второй миграции (→ `IrreversibleMigration`), физическое удаление файла обложки в галерее (галерея
переведена на тот же маппинг `wardrobe_item_photo` + `delete_on_update/remove: false`), скрытая
форма-заглушка ради старого delete-теста, дубль vich-маппинга, две миграции на одну колонку
(сведены в `Version20260726_wardrobe_admin_styles`, `main_style` переименована), удалённое из формы
поле «Любовь с первого взгляда» (возвращено), 🌶 в `STYLE_LABELS`, `<select multiple>` → `expanded`,
«Не указано» как магический ключ (→ `key === null`), полная гидрация в статистике (→ SQL-агрегат),
пустые фильтры в ссылках (→ `activeFilters`).

### Что осталось из ревью (мелочи, не блокеры)
- [ ] `WardrobeItemRepository::applyFilters()` — вариативный регистр в LIKE: 4 варианта запроса × 5
  полей = до 20 OR-клауз, сделано ради SQLite тестового контура (комментарий в коде это признаёт).
  Прод-MySQL `utf8mb4_unicode_ci` и так регистронезависим, а приём неполный: `ЛЬНЯНАЯ РУБАШКА` →
  title-case «Льняная Рубашка» ≠ «Льняная рубашка». Заменить на одну клаузу `LOWER(w.field) LIKE :q`
  с lowercase-параметром (одинаково работает в обеих БД) + тест с запросом в верхнем регистре.
- [ ] `WardrobeItemRepository::findActiveForUser()` — мёртвый метод после перехода index на
  `searchForUser()` (в `src/` и `templates/` осталось только определение). Удалить.
- [ ] `getFilterOptions()` — 5 отдельных DISTINCT-запросов на каждый показ списка, даже без фильтров.
- [ ] Строка `given_away` в «Статусе носки» на экране статистики: число видно, а перейти к этим вещам
  нельзя ни фильтром, ни ссылкой (исключены и из активного списка, и из архива).
- [ ] DB-инвариант «один активный default-гардероб на владельца» — см. пункт выше в разделе
  Wardrobe MVP (строка ~1778), гонка web + Telegram всё ещё возможна.

### Дыры самого контура ревью (важнее остатков полировки)
- [ ] **Быстрые мержи проскакивают.** #45 жил 45 секунд, #49 — меньше 30 минут; за ночь 28.07 в
  `main` ушли #45, #49–#53 **без ревью**, каждый с автодеплоем на прод (все прогоны CI зелёные).
  Опрос раз в 30 минут против такого бессилен by design. Варианты: workflow на `pull_request`,
  который дёргает ревью; ревью постфактум по коммитам `main`; либо принять как норму для своих PR.
- [ ] **`allow_auto_merge: false` в настройках репо** → `gh pr merge --auto` отвергается, страховка
  «домержить только после зелёного CI» не работает; #44 пришлось мержить без `--auto`. Включить
  auto-merge в настройках, если механика нужна.
- [ ] **Договорённость про Draft.** Неготовое надо открывать как Draft: `isDraft` — единственный
  однозначный машинный признак «не готово» (за всю историю репо не использован ни разу, 0 из 60 PR).
  Фильтр готовности монитора уже умеет: draft, `WIP`/`Draft` в заголовке, метки
  `wip`/`do-not-merge`/`blocked`/`on-hold`, последний коммит младше 10 минут. Правило стоит дописать
  в `AGENTS.md` рядом с branch-per-change (Rule 5), чтобы агенты открывали черновики сами.
- [ ] **Стек feature-веток без CI.** #26/#41/#44/#46 были отрезаны от `main` до мержа #25, поэтому
  `.github/workflows/ci.yml` на них отсутствовал и **ни один PR стека не проверялся в CI** — только
  локальный прогон автора. При следующем стеке ребейзить на актуальный `main` сразу.
- [ ] **Squash + стек = дубли.** #46 приехал `CONFLICTING`, потому что нёс вторую копию работы #44:
  после squash-мержа история родительской ветки не становится предком базы. Каждый следующий PR в
  стеке обязан подтянуть базу сразу после мержа предыдущего.
