...
- [ ] **Физические магазины бренда (brand_store) — вывести и дать управлять**: данные есть (1449 точек у 744 брендов из enrichment, source='enrichment'), но «спящие» — нигде не отображаются и не редактируются. Сделать: (а) блок «Магазины» на странице бренда showv2 (адрес/город/часы/телефон) + LocalBusiness/Store JSON-LD; (б) CRUD в кабинете бренда (BrandLk) — внесение/правка точек владельцем; (в) CRUD в админке (EasyAdmin); (г) включить stores в payload агент-пуша на прод; (д) валидация enrichment-точек (омонимы — кейс Zatmenie: адрес ночного клуба). FAQ уже использует адреса как факты (2026-06-04).
...
**Осталось для боевого запуска:** деплой кода на прод + миграции там; env прода: AGENT_API_TOKEN/SECRET (сгенерить боевые!), PUBLISH_LAUNCH_DATE; cron `0 * * * * app:brand:publish-tick`; на dev: PROD_API_URL=https://wearbase.ru + боевые токены; стадию push включать в сетевой демон ТОЛЬКО после деплоя прода; GPU-демон перезапустить с faq-стадией (--stages=embed:30,generate:10,faq:10).
