...
**Осталось для боевого запуска:** деплой кода на прод + миграции там; env прода: AGENT_API_TOKEN/SECRET (сгенерить боевые!), PUBLISH_LAUNCH_DATE, INDEXNOW_KEY (ключ-файл public_html/{key}.txt уже в репо); cron `0 * * * * app:brand:publish-tick`; на dev: PROD_API_URL=https://wearbase.ru + боевые токены; стадию push включать в сетевой демон ТОЛЬКО после деплоя прода; GPU-демон перезапустить с faq-стадией (--stages=embed:30,generate:10,faq:10).
