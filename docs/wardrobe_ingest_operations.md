...
- каталог приватных загрузок недоступен для записи и не может быть создан (отсутствие каталога само
  по себе не критично: Vich создаёт его при первой загрузке, а deploy-шаг делает `mkdir -p`).
