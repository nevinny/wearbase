...
> **UPD 2026-07-07, генерация:** промпты `LlmService::generateListicle()/generateGuide()` получили параметр `noTables` (команды передают его для платформы dzen) — новые Дзен-копии генерятся со «Сравнением брендов» списком, без markdown-таблиц. `toDzenHtml()` в фиде остаётся страховкой для старых копий.
>
