...
| [internal_linking_graph_playbook.md](internal_linking_graph_playbook.md) | Граф перелинковки брендов: эффект по GSC/Вебмастеру (не доказан), грабли «богатые богатеют», плейбук переноса + как мерить (holdout) |
...
| [seo_competitor_content.md](seo_competitor_content.md) | **Разведка конкурентов в выдаче**: SERP по фразе (Yandex Search API) → скрейп статей → извлечение тем/структуры (не фактов — юр. риск) → `--gap-context` в уже существующие `listicle`/`guide`/`replace-listicle` (без нового генератора). Гейт по интенту (`AioQueryClassifier`), 2 новые сущности (`CompetitorArticle`, `SeoCompetitorScan`), anti-duplicate через `NearDuplicateDetector`. Проектирование, реализация не начата. От 2026-09-23 |
