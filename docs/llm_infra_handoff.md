...
### Шум и охлаждение GPU (замеры 2026-09-03)

Тюнинг карт держит root-юнит `rig-gpu-tune.service` (в `/etc/systemd/system/`, `enabled`):
power-лимиты 2060S 140 W ×3 / A4000 135 W + демон фан-кривой `~/miner/fanctl.py auto`.

⚠️ **Приглушить карты на простое нельзя** — у каждой свой аппаратный пол оборотов, ниже него
драйвер клампит команду. GPU0/GPU2 (VBIOS `90.06.4C.00.7A`) — 41%, A4000 — 30%. Тише станет
только перешивкой VBIOS на `…65` (как у GPU1, борд идентичен) — необратимо, отдельное решение.

⚠️ **У GPU1 вентилятор не управляется**: 0% и при запросе 0%, и при 90%, при этом карта греется
под ollama. Требует физического осмотра — под нагрузкой уйдёт в троттлинг.

Полные замеры и закрытые гипотезы (правка кривой на `0%`, драйверный zero-RPM, корпусные PWM —
всё не работает) — в скилле `.claude/skills/mining/SKILL.md`, раздел «Вентиляторы».
