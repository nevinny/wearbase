...
> Последний осмотр по SSH: **2026-09-13, около 07:43 UTC**. Сервер включён; Ollama,
> Qdrant и SearXNG отвечают. Исторические сведения ниже отдельно датированы.

## 1. Доступ

- **SSH:** `ssh zyablik@192.168.2.43` (проверено 2026-09-13).
- На Mac владельца в `~/.zshrc` добавлен `alias llm='ssh zyablik@192.168.2.43'`.
  Быстрый вход: `llm`; в уже открытом терминале сначала `source ~/.zshrc`.
  Это shell-алиас, а `ssh llm` в исторических примерах требует отдельного `Host llm`
  в `~/.ssh/config`. Пароль вводится интерактивно и в документации не хранится.
...
|---|---|---|
| GPU | 2× RTX 2060 SUPER (8 GiB) + RTX A4000 (16 GiB) + 2× CMP 30HX (6 GiB) ≈ **44 GiB VRAM** | Ollama использует первые три карты; CMP при осмотре без процессов |
| CPU | Intel i7-2670QM, 4C/8T @ 2.2 GHz | старый (Sandy Bridge), CPU-инференс медленный |
| **RAM** | **7.6 GiB всего**, 2.5 GiB available; swap 316 MiB / 4 GiB | 🔴 **главный потолок** — модели, спиллящие в RAM, не влезают |
| Диск | 218 GiB, занято 80%, доступно 43 GiB | перед `ollama pull` проверять `df -h` |

ОС: Ubuntu 24.04.4 LTS, ядро `6.8.0-110-generic`, hostname `llm`.
Драйвер NVIDIA `595.58.03` (`nvidia-smi` показывает CUDA 13.2).
На момент осмотра uptime 1 день 21 час, load average 0.09 / 0.07 / 0.08.
Температуры GPU 31–47 °C, загрузка GPU 0%.

### Шум и охлаждение GPU (исторический разбор 2026-09-03)

На 2026-09-13 состав GPU изменился: сведения ниже о трёх 2060S и неисправном
вентиляторе GPU1 относятся к прежнему осмотру. Сейчас GPU1 показывает 42%; факт ремонта
не проверялся, индексы карт нельзя считать постоянными. `rig-gpu-tune.service` работает;
наблюдаемые power limits: 2060S 140 W ×2, A4000 140 W, CMP 125 W ×2.
...
**Про «добавить диски» (историческая оценка 2026-08-13, +2×128 GB; сейчас свободно 43 GiB).** На тот момент 110 GB
...
Проверено командами `ollama list` и `ollama ps` 2026-09-13.
Имена ниже — локальные теги, размеры — вывод `ollama list`.

| Модель | Размер на диске | Состояние при осмотре |
|---|---|---|
| **`gemma4:26b`** | 17 GB | загружена, runtime 18 GB, 4%/96% CPU/GPU, контекст 8192, UNTIL Forever |
| `qwen3.8:27b` | 16 GB | установлена |
| `qwen3.6:27b` | 17 GB | установлена |
| `qwen3.5:27b` | 17 GB | установлена |
| **`qwen3-embedding:0.6b`** | 639 MB | установлена; исторически выход 1024-dim |

Процесс Ollama занимает около 6.4 / 4.1 / 8.0 GiB на GPU 0 / 1 / 2.
Обе CMP 30HX показывают 0 MiB; их пригодность для инференса не проверялась.
Capabilities, генерация, vision, эмбеддинги и скорость в этом осмотре не тестировались.
Проверка capabilities: `llm 'ollama show <model>'`.
Значение `WARDROBE_VISION_MODEL` и работоспособность relay не проверялись.
...
Исторический snapshot 2026-07-15. Состав, число точек и авторизация не перепроверялись
2026-09-13; проверен только `/healthz`.
...
## 9. Рабочие каталоги сервера

При осмотре 2026-09-13 в `/home/zyablik` найдены `wearbase`, `llm-shim`, `models`,
`vton`, `vton-garments`, `miner`, `qwen38`, `tools`, `bin`.
Наличие каталогов не подтверждает, что соответствующие приложения запущены.
Осмотр был диагностическим: конфигурация сервера не менялась, нагрузочные тесты не запускались.
