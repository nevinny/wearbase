...
драйвер клампит команду. GPU0/GPU2 (VBIOS `90.06.4C.00.7A`) — 41%, A4000 — 30%. Перешивка VBIOS
рассматривалась и **отклонена владельцем 2026-09-03** (BIOS не трогаем). Остаётся только
физический путь — замена вентиляторов: пол задаётся в % ШИМ, тихий вентилятор на тех же 41%
шумит меньше.
...
но ollama кладёт на неё модели — под нагрузкой уйдёт в троттлинг.
