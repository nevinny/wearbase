...
## Промпт для codex (скопировать целиком)

> Прочитай `docs/onboarding.md` полностью и подними проект локально по нему: проверь
> требования, выполни шаги запуска (composer, локальный MySQL, `.env.local` из
> `.env.local.example`), импортируй присланный дамп `wearbase-dev-lite.sql.gz` вместо
> `doctrine:migrations:migrate` (см. раздел про дамп), запусти `symfony serve` и убедись,
> что главная и каталог отдают 200. Я работаю над гардеробом: настрой AI-подсказки на
> **локальную vision-модель** по разделу «Разработка гардероба» (ollama + `WARDROBE_VISION_LOCAL=1`),
> OpenRouter не используем. Если чего-то не хватает (нет дампа, порт занят, миграция падает) —
> остановись и спроси, не выдумывай обходные пути и не переводи проект на SQLite.

Дамп БД (`wearbase-dev-lite.sql.gz`, ~16 МБ) приходит отдельно — через git он не передаётся.
